/*
 * This file is part of HuDK.
 * ASM and C open source software development kit for the NEC PC Engine.
 * Licensed under the MIT License
 * (c) 2016-2020 MooZ
 */
#include "hudk.h"

#include "data/map_16x16.h"

#incbin(map_00, "data/map_16.map")
#incbin(gfx_00, "data/map_16x16.bin")
#incbin(tile_pal_00, "data/map_16x16.idx")
#incbin(pal_00, "data/map_16x16.pal")

// sine table [-128,128[
const char sin_table[256] = {
    
};

int scroll_x;
int scroll_y;
char sin_idx;
char map_col;

void main() {
    irq_disable(INT_ALL);

   // set BAT size
    vdc_set_bat_size(VDC_BG_64x32);

    // set map bounds
    map_set_bat_bounds(0, vdc_bat_height);

    // load tileset palette
    vce_load_palette(0, 2, pal_00);

    // load tileset gfx
    vdc_load_data(0x2200, gfx_00, 512);

    // set map pointer, tiles vram address, tiles palette id, map width and height, and wrap mode
    map_set(map_00, map_16x16_tile_vram, tile_pal_00, map_16x16_width, map_16x16_height);

    // copy map from (0,0) to (16, map_height) to BAT
    map_load_16(0, 0, 0, 0, 17, map_16x16_height);

    // the last map column
    map_col = 16;
    
    // reset index in the sine table
    sin_idx = 0;

    // set scroll window
    scroll_set(0, 0, 254, 0, 0, 0x89);

    // enable IRQ 1.
    irq_enable(INT_IRQ1);

    for(;;) {
        // wait for vsync
        vdc_wait_vsync();

        ++scroll_x;

        // this is the wavy scroll effect
        // the Y coordinate is computed like this :
        //    Y = (sin_table[sin_idx] / 2) + 64
        scroll_y = sin_table[sin_idx];
        if(scroll_y & 0x0080) {
            scroll_y |= 0xff00;
        }
        scroll_y >>= 1;
        scroll_y += 64;

        ++sin_idx;     // we move to the next sine value

        // update scroll coordinates
        scroll_set_coord(0, scroll_x, scroll_y);

        // check if we need to load a map column
        // note the only go from left to right
        // and we'll never need to load more that a single column
        if((scroll_x & 15) == 0) {
            ++map_col;
            if(map_col >= map_16x16_width) {
                map_col = 0;
            }
            // copy the next map column to BAT
            map_load_16(map_col, 0, map_col, 0, 1, map_16x16_height);
        }
    }
}
